...
const QuestionTally = ({globalStats}) => {
  ...
  return (
    <section className="bg-card border shadow-md border-card-border rounded-lg p-4 flex flex-col justify-center items-center text-center">
      <h2 className="text-lg font-semibold mb-2 text-muted">Problems Solved</h2>
      <hr className="text-muted w-full h-1.5"/>
      <h2 className="font-semibold mb-2 text-muted">Data Structures and Algorithms</h2>
      <section className="w-full flex justify-between items-center">
        <div className="w-[300px] relative">
          <ResponsiveContainer width="100%" aspect={1}>
            <PieChart>
              <Pie data={globalStats.difficulty || []} dataKey="value" cx="50%" cy="50%" innerRadius={50} outerRadius={70} paddingAngle={3}> {globalStats.difficulty?.map((entry, index) => ( <Cell key={`cell-${index}`} fill={COLORS[index]} /> ))}</Pie> <Tooltip />
            </PieChart>
          </ResponsiveContainer>
          <div className="absolute inset-0 flex items-center justify-center"><span className="text-3xl font-bold">{globalStats.total_dsa_solved}</span></div>
        </div>
        <div className="w-full flex flex-col justify-center items-center gap-2">
          {globalStats.difficulty?.map((item, index) => (
            <div key={index} className="w-full flex justify-between items-center font-bold rounded-xl bg-bg px-4 py-1"><span style={{"color": COLORS[index]}}> {item.name}</span><span>{item.value}</span></div>
          ))}
        </div>
      </section>
      <hr className="text-muted w-full h-1.5"/>
      <h2 className="font-semibold mb-2 text-muted">Competitive Programming</h2>
      <section className="w-full flex justify-between items-center">
        <div className="w-[300px] relative">
          <ResponsiveContainer width="100%" aspect={1}>
            <PieChart>
              <Pie data={globalStats.cp_difficulty || []} dataKey="value" cx="50%" cy="50%" innerRadius={50} outerRadius={70} paddingAngle={3}> {globalStats.cp_difficulty?.map((entry, index) => ( <Cell key={`cell-${index}`} fill={COLORS[index]} /> ))}</Pie> <Tooltip />
            </PieChart>
          </ResponsiveContainer>
          <div className="absolute inset-0 flex items-center justify-center"><span className="text-3xl font-bold">{globalStats.total_cp_solved}</span></div>
        </div>
        <div className="w-full flex flex-col justify-center items-center gap-2">
    ...
    <section className="bg-card border shadow-md border-card-border rounded-lg p-4 grid grid-cols-[1fr_2fr] justify-center items-center text-center gap-2">
      ...
      <div className="w-full flex flex-col justify-center items-center gap-1">
        {idvStats?.map((item, index) => (item.contests > 0 && (
          <button key={index} className={`w-full flex justify-between items-center font-bold ${curPlatform === index ? 'border' : 'opacity-90'}`} style={{backgroundColor: 'var(--color-bg)', color: 'var(--color-text)'}} onClick={() => setCurPlatform(index)}><span className="flex justify-center items-center gap-4"><img src={platformIcons[item.platform]} alt={item.platform} className="w-5 h-5" /><span className="capitalize">{item.platform}</span></span><span>{item.contests}</span></button>
        )))}
      </div>
    </section>
...
const RankTally = ({idvStats, links}) => {
  ...
  return(
  <section className="bg-card border shadow-md border-card-border rounded-lg p-4 flex flex-col justify-center items-center text-center">
    <h2 className="text-lg font-semibold mb-2 text-muted">Contest Rankings</h2>

    <hr className="text-muted w-full h-1.5"/>
    <h2 className="font-semibold mb-2 text-muted uppercase">{idvStats?.[0]?.platform}</h2>
    <section className="w-full grid grid-cols-[1fr_2fr] md:grid-cols-2 items-center font-bold rounded-xl px-3 md:px-10 py-1 mb-3 gap-5 md:gap-0">
      <img src={rankImg[idvStats?.[0]?.platform]?.[idvStats?.[0]?.rank_title]} alt={idvStats?.[0]?.platform} className="w-25 h-25" />
    ...
    <hr className="text-muted w-full h-1.5"/>
    <h2 className="font-semibold mb-2 text-muted uppercase">{idvStats?.[1]?.platform}</h2>
    <section className="w-full grid grid-cols-[1fr_2fr] md:grid-cols-2 justify-between items-center font-bold rounded-xl px-3 md:px-10 py-1 mb-3 gap-5 md:gap-0">
      <p className='w-full capitalize text-3xl text-start' style={{ color: rankImg[idvStats?.[1]?.platform]?.[idvStats?.[1]?.rank_title] }}>{idvStats?.[1]?.rank_title}</p>
      <div className="flex flex-col justify-start text-start gap-1">
    ...
    <hr className="text-muted w-full h-1.5"/>
    <h2 className="font-semibold mb-2 text-muted uppercase">{idvStats?.[2]?.platform}</h2>
    <section className="w-full grid grid-cols-[1fr_2fr] md:grid-cols-2 justify-between items-center font-bold rounded-xl px-3 md:px-10 py-1 mb-3 gap-5 md:gap-0">
      <p className="flex gap-1">
        {Array.from({length: parseInt(idvStats?.[2]?.rank_title) || 0}).map((_, index) => (
          <span key={index} className="w-8 h-8 flex items-center justify-center text-white text-xl rounded" style={{backgroundColor: rankImg[idvStats?.[2]?.platform]?.[idvStats?.[2]?.rank_title]}}>{idvStats?.[2]?.rank_title?.[1]}</span>
        ))}
      </p>
      <div className="flex flex-col justify-start text-start gap-1">
    ...
    <hr className="text-muted w-full h-1.5"/>
    <h2 className="font-semibold mb-2 text-muted uppercase">{idvStats?.[3]?.platform}</h2>
    <section className="w-full grid grid-cols-[1fr_2fr] md:grid-cols-2 justify-between items-center font-bold rounded-xl px-3 md:px-10 py-1 mb-3 gap-5 md:gap-0">
      <p className='w-full uppercase text-3xl md:pr-19' style={{ color: idvStats?.[3]?.rank_title }}>{idvStats?.[3]?.current_rating}</p>
      <div className="flex flex-col justify-start text-start gap-1">
